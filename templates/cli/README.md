...
`generate_export_header(`, and this project has no such call: `mycli_lib` is
static and exists so the tests have something to link. So it is grey on every
pull request, and grey reads like absent without being it - required status
checks count a skipped job as a passing one, so naming that one gets you a
check that builds nothing and reports success. It has something to say the day
this project installs a shared library, and nothing until then.
